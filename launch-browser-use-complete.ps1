...
function Test-Prerequisites {
    ...
    try {
        ...
        Write-Error "Python not found: $($_.Exception.Message)"
    }
    ...
    try {
        ...
        if ($LASTEXITCODE -eq 0) {
            ...
            Write-Error "Git not found or not in PATH"
        }
    }
    catch {
    ...
    if ($UseDocker) {
        try {
            ...
            if ($LASTEXITCODE -eq 0) {
                ...
                Write-Error "Docker not found but UseDocker specified"
                return $false
            }
        }
        catch {
            Write-Error "Docker not found: $($_.Exception.Message)"
            return $false
        }
    }
...
function Clone-Repository {
    ...
    try {
        ...
        if ($LASTEXITCODE -eq 0) {
            ...
            Write-Error "Failed to clone $DisplayName"
            return $false
        }
    }
    catch {
    ...
    try {
        if (!$SkipDependencies) {
            ...
            if ($LASTEXITCODE -eq 0) {
                ...
                Write-Error "Failed to install browser-use dependencies"
                return $false
            }
        }
    ...
    try {
        if (!$SkipDependencies) {
            ...
            if ($LASTEXITCODE -eq 0) {
                ...
                Write-Error "Failed to install workflow-use dependencies"
                return $false
            }
        }
...
function Start-WebUI {
    ...
    if (!(Test-Path $webUIPath)) {
        Write-Error "Web-UI not found at $webUIPath"
        return $null
    }
    ...
    try {
        ...
        if ($process) {
            ...
            Write-Error "Failed to start Web-UI process"
            return $null
        }
    }
    catch {
        Write-Error "Error starting Web-UI: $($_.Exception.Message)"
        return $null
    }
    finally {
        Pop-Location
...
function Start-WorkflowBackend {
    ...
    if (!(Test-Path $workflowPath)) {
        Write-Error "Workflow-Use not found at $workflowPath"
        return $null
    }
    ...
    try {
        ...
        if ($process) {
            ...
            Write-Error "Failed to start Workflow backend process"
            return $null
        }
    }
    catch {
        Write-Error "Error starting Workflow backend: $($_.Exception.Message)"
        return $null
    }
    finally {
        Pop-Location
...
function Main {
    try {
        ...
        if (!$setupSuccess) {
            Write-Error "Setup failed for one or more components"
            exit 1
        }
